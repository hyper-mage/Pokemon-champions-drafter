# Requirements Archive: v1.0 Full Draft and Tournament
...
**92 of 94 v1 requirements Complete.** Checkboxes below are the authoritative per-requirement
record and agree with the traceability table at the end of this file. The two unticked entries
are deliberately unticked — they did not ship Complete, and ticking them to tidy the archive
would falsify it:

| Requirement | Status | Why |
|-------------|--------|-----|
| `BAN-07` duplicate-ban policy | **Partial** | Owner decision D-19. `bothApply` is built; the `Re-ban` arm ships present-but-disabled so a later milestone enables an option rather than adding a control plus a schema bump. `duplicateBanPolicy` is written by `ConfigScreen.tsx`, validated at `import-guard.ts` and migrated by `migrate.ts`, but read by no reducer — documented as deliberate at `model.ts:330-343`. |
| `EXPO-04` Showdown validator pass | **Pending** | Requirement-text defect, not a code gap. No species-only paste can satisfy *"passes its team validator"*: Showdown reports four inherent problems per Pokémon (no ability, no moves, 0 stat points) regardless of implementation correctness. The achievable discriminating signal (no `transforms in-battle` error) is verified against the real `pokemon-showdown@0.11.11` parser in `docs/export-verification.md`. Needs a reword next milestone, not a closure phase. |

Two requirements were **amended in flight** rather than left stale — `ROST-03` and `ROST-05`,
both reworded because the build pins an exact npm package version plus its sha512 integrity hash
rather than a git commit SHA. The original wording named a SHA the build never checks out. The
amendments are recorded inline in the requirement text below.

Coverage was computed from each phase’s `VERIFICATION.md` requirements table, which is complete
for all 94. See `v1.0-MILESTONE-AUDIT.md` for the three-source cross-reference.

**`.planning/REQUIREMENTS.md` was removed at this milestone’s close.** This file is the record
for v1.0; the next milestone writes a fresh one via `/gsd-new-milestone`.
