...
This is the ROADMAP.md as it stood at milestone close, preserved verbatim below.
For the shipped summary, decisions and known gaps, see `.planning/MILESTONES.md`.
...
**Decimal Phases:** None. All five phases were planned integers; nothing was inserted urgently.

**Key Decisions (with outcomes):**

- Append-only action log plus a pure reducer — ✓ Good. A full tournament is 350–500 actions, so a
  complete re-fold is sub-millisecond. Bought undo, the recap, correct-by-construction export and
  sync-readiness at no measured cost.
- Undo designed in from line one — ✓ Good. Never retrofitted; `isUndoable` covers every Phase 2–5
  action kind and `undoAnnouncement` uses a `const exhaustive: never` default so a missing arm is a
  compile error rather than a silent gap.
- Composition rules compile to round structure — ✓ Good. The strongest idea in the project. Typed
  slots made invalid teams unrepresentable and let the runtime validator be deliberately removed.
- Compiled rounds type the team slots — ✓ Good, and load-bearing. The slot decides the export
  stone, never the species.
- Serializable single-object tournament state — ✓ Good. No `Set`, `Map`, `Date` or class instance
  reaches a persisted structure; verified through the v1→v5 migration chain and the import guard.
- Config-time feasibility solver — ✓ Good. RULE-07 general arithmetic in Phase 2, RULE-09 Mega
  feasibility in Phase 3 as research predicted. No configuration this build accepts can open a
  Mega round with nothing in it.
- Export is species-only except `Species @ Stone` for Mega slots — ✓ Good. The Phase 1 spike
  settled the one untested assumption: pokebase.app interprets the `@ Stone` line rather than
  merely tolerating it, so one paste string serves both targets.
- Snapshots are regulation-stamped and the script pins a SHA — ✓ Good, and it paid off inside the
  milestone: two regulations shipped (M-A 213 entries, M-B 235) and an archived tournament stays
  readable across the rotation.
- Priority cards: open sequential, count derived from rounds — — Pending. Built and tested, but the
  mechanic's *feel* (D-18 rotation advantage, D-23 low-plays-first) is unvalidated until the beta
  playtest runs.
- Host picks the duplicate-ban policy — ⚠️ Revisit. Partial by decision D-19: `bothApply` built,
  `Re-ban` ships present-but-disabled so a later milestone enables an option rather than adding a
  control plus a schema bump.
- Hot-seat on one screen, no networking — ✓ Good so far. The single write path through `dispatch`
  held; the `BroadcastChannel` tab lock is the only ambient coordination in the codebase.

**Issues Resolved:**

- The 207-vs-208 base-species discrepancy and the Mega-capable count, settled against the pinned
  `pokemon-showdown@0.11.11` snapshot rather than left as folklore.
- The pokebase `@ item` assumption, converted from inference to a tested fact by the Phase 1 spike.
- The original priority-card spec's internal contradiction, which produced ties in 98.5% of rounds
  at 6 players with no tiebreak rule.
- Phase 4's BLOCKING physical secrecy check, batched into Phase 5's gate (05-15 Part 2a) and passed
  2026-09-01 — clearing BAN-05 and BAN-06 acceptance alongside DRFT-14.
- Milestone bookkeeping: REQUIREMENTS.md checkboxes reconciled to its own traceability table
  (73→92 of 94), STATE.md progress corrected to 5 phases / 100%.

**Issues Deferred:**

- The card-mechanic playtest with real players — deferred to beta by host decision 2026-08-19,
  re-confirmed 2026-08-20. The sole item behind Phase 3's `human_needed` status. Both mechanics it
  would test are a one-line change each, so deferring costs little.
- Screen-reader verification — DESCOPED by host decision 2026-08-20 (no screen reader configured).
  A documentation gap, not a functional risk: every fact is also carried by a focused control's own
  name or by persistent on-screen text.
- BAN-07's `Re-ban` arm — see D-19 above.
- EXPO-04's requirement text — needs a reword next milestone, not a closure phase.

**Technical Debt Incurred:**

- 24 warnings and 16 info findings open across Phases 1–3; Phases 4 and 5 carry none.
- `WR-09`: heartbeat during the claim window is not counted, allowing a narrow dual-ownership
  write window. `WR-10`: a bfcache-restored secondary tab loses its stale watch permanently.
- `tests/build/sw-manifest.test.ts` and `tests/ui/ban-list.test.tsx` are flaky under full-suite
  parallel load; neither failed in the audit-time run.
...
- Nine pre-existing roster figures hardcoded in `src/` doc comments.
- The config screen has no way back to the landing screen.
- Two `PoolGrid`s on the config screen keep independent densities.
