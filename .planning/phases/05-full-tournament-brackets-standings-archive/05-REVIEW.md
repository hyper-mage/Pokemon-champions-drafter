...
carried_forward:
  ...
  - "IN-02 verb: `1 match were voided.` keeps the copy table's plural VERB while fixing its
    numeral, matching RecapList.voidLine's documented posture that a component quietly fixing the
    verb would put the three sentences out of agreement. The verb is the copy table's to amend, not
    a component's. Asserted byte-for-byte by test."
