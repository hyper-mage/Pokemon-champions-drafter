...
gaps:
  - truth: "In blind mode, each player enters their bans behind a real full-screen pass-the-device interstitial — not an input mask (success criterion 2, BAN-05)"
    status: resolved
    resolved_by: "a603e55 — fix(04-12): give the blind entry surface its own full-screen shell"
    resolved_at: 2026-08-25T00:00:00Z
    resolution: "`app.tsx`'s shell expression gained a fourth arm emitting a new full-bleed `.entry-shell` class (no `max-width`, no `margin-inline`, no padding), and `BanStageScreen` reports the entry sub-state up to the composition root as a BOOLEAN from a layout effect — who is entering and what they have chosen stay in the component, because D-18 requires the in-progress selection to die with it, and the layout effect is what makes the class flip before paint rather than a frame late. The surface deliberately stays INSIDE the `inert` gate: the sibling-render route this report's own `missing` list suggested first would hand a read-only tab a live, interactive ban screen, reopening T-04-20 with a secrecy problem attached. `BlindEntry.css`'s stale contract comment (the review's IN-01) is corrected in the same commit and now records the shell it depends on. Two cases in `tests/ui/read-only-shell.test.tsx` close the missing-test half: one pins `.entry-shell` for the entry sub-state and asserts `.app-shell`/`.draft-shell` are both absent, the other asserts the surface and its `Lock in` control are under the gate in a read-only tab while the takeover button is not. Both were proved able to fail — reverting the shell arm breaks the first (`app-shell` where `entry-shell` is required), and moving the ban stage outside the gate breaks all three ban-stage containment cases. Neither case touches the locked or reveal states, so 04-09 remains the only authority on those. `npm run verify` exits 0 at `032e4fa` (61 files, 2040 tests)."
...
| 2b | Blind mode's entry surface is a real full-screen interstitial, not an input mask | ✓ RESOLVED (`a603e55`) | Was ✗ FAILED at first verification: `BlindEntry` mounted inside `.app-shell` (1200px cap, `app.css:73-78`) rather than on its own full-screen surface, contradicting `04-UI-SPEC` §3/§5. Closed by `a603e55` — a fourth arm on `app.tsx`'s shell expression emitting `.entry-shell` (full-bleed, no page padding), with the surface still under the `inert` gate, plus the two shell-containment cases in `tests/ui/read-only-shell.test.tsx` that this report found missing. See the frontmatter gap's `resolution` for the full account. |
...
**Score at first verification:** 5 truths fully VERIFIED, 1 FAILED (2b), 1 owner-overridden (4b) = **3 of 4 roadmap success criteria fully met; criterion 2 has an unresolved code gap; criterion 4 has an accepted, deliberate partial.**

**After the follow-up fixes (`a603e55`, `032e4fa`):** 6 truths VERIFIED, 0 failed, 1 owner-overridden (4b). Criterion 2's gap is closed and criterion 3's WR-02 warning is closed. Criterion 4's remaining partial is unchanged and stays an accepted override (D-19). This line records the code state; the frontmatter `status` field is the re-verification's to set, not this edit's.
...
| `src/ui/components/BlindEntry.tsx` | Full-screen entry surface | ✓ VERIFIED (since `a603e55`) | Component was already functionally complete (typeahead, chip list, capped grid, sticky footer, no `announce` import, no motion); what was missing was the shell around it. `a603e55` adds `.entry-shell` and the composition-root wiring that selects it, so the "full-screen" artifact-level contract is now met by the composed result rather than only claimed by the component's own comment. |
| `src/adapters/ban-shield.ts` | `installBanShield`, sentinel history entry, teardown ordering | ✓ VERIFIED | Confirmed at source; listener removal precedes sentinel consumption as documented. WR-02's async race, noted as a WARNING under criterion 3, is closed by `032e4fa`. |
...
| `app.tsx` shell wrapper | `BlindEntry.tsx` | Its own full-screen surface (per 04-UI-SPEC §3) | WIRED (since `a603e55`) | Was **NOT WIRED**: `BlindEntry` inherited `.app-shell`'s 1200px cap. `a603e55` adds the `.entry-shell` arm and the boolean `BanStageScreen` reports up, and `tests/ui/read-only-shell.test.tsx` now pins both the class and the fact that the surface stays under the `inert` gate. |
...
| `BlindEntry`/blind-entry sub-state has no shell-containment test | `grep -n "app-shell\|draft-shell" tests/ui/blind-entry.test.tsx tests/ui/ban-stage.test.tsx` | No matches | ✗ FAIL at first verification. Closed by `a603e55`: the guard lives in `tests/ui/read-only-shell.test.tsx` (`describe('the shell over the blind entry surface')`) rather than in either file grepped here, because the shell element belongs to the composition root and neither of those files mounts it. |
...
| BAN-05 | 04-09, 04-10, 04-12 | Blind mode uses a full-screen pass-the-device interstitial, not an input mask | ✓ SATISFIED (since `a603e55`) | The "no top bar/turn banner/panes while entering" half was always met (`BlindEntry` renders none of those). The "full-screen surface, not an input mask" half was ✗ BLOCKED at first verification — `BlindEntry` was nested in `.app-shell`'s 1200px, padded, centred column (WR-01) — and is closed by `a603e55`'s `.entry-shell`. |
| BAN-06 | 04-10, 04-12 | Returning via the back button cannot resurrect a private ban screen | ✓ SATISFIED | `ban-shield.ts` sentinel + `popstate` handling, fixed in `ec75b4b`, host-re-verified. WR-02's async-race warning never amounted to resurrection and is now closed outright by `032e4fa`. |
...
|------|------|---------|----------|--------|
| `src/app.tsx` / `src/ui/screens/BanStageScreen.tsx` / `src/ui/components/BlindEntry.css` | 2105-2119 / 744-755 / 31-44 | Shell containment does not match the design contract (WR-01) | 🛑 Blocker (for success criterion 2) — **RESOLVED in `a603e55`** | The phase's own "most important screen" shipped as a centred, padded column rather than the specified full-bleed interstitial. Now on its own `.entry-shell`, still inside the `inert` gate, with two containment cases pinning both facts. |
| `src/adapters/ban-shield.ts` | 157-168 | Async `history.back()` in teardown not guarded against a stale sentinel read by a subsequent fast install (WR-02) | ⚠️ Warning — **RESOLVED in `032e4fa`** | Narrow race window bounded by human reaction time; did not resurrect a private screen but could falsely discard the next player's in-progress entry under rapid succession. Closed by a module-level `consumeInFlight` flag raised before the traversal is requested, so an installation adopting a sentinel that is already being spent swallows exactly one `popstate` and re-pushes its own entry. |
| `src/ui/components/BlindEntry.css` | 38-40 | Comment ("The surface IS the screen...fills the viewport") is inaccurate given WR-01 | ℹ️ Info — **RESOLVED in `a603e55`** | Stale contract comment. Now true of the composed result, and rewritten to name the `.entry-shell` ancestor it depends on so the coupling cannot be broken silently a second time. |
...
### Gap Resolution — added after the follow-up fixes

Both items above were closed on `main` after this report was written, as two separate commits:

| Item | Commit | What changed |
|------|--------|--------------|
| WR-01 / criterion 2's gap / the missing shell test / IN-01 | `a603e55` | `app.tsx` gains a fourth shell arm and a new full-bleed `.entry-shell` class; `BanStageScreen` reports the entry sub-state up as a boolean from a layout effect; `BlindEntry.css`'s contract comment corrected; two shell-containment cases added to `tests/ui/read-only-shell.test.tsx`. |
| WR-02 (the tracked warning) | `032e4fa` | `ban-shield.ts` gains a module-level `consumeInFlight` flag raised before teardown requests its traversal, so a later installation adopting a not-yet-spent sentinel swallows exactly one `popstate` and re-pushes its own entry; two cases added to `tests/adapters/ban-shield.test.ts` with a `queueTraversals` helper that models a real browser's queued traversal. |

`npm run verify` exits 0 at `032e4fa` — `check:pure` 0 violations, `check:nohtml` 0 violations,
61 test files / 2040 tests passing, `tsc` clean on both projects, `vite build` and the
service-worker manifest (322 URLs) both succeeding.

**Not re-scored here.** This section records what changed and where; the frontmatter `status`
field and the phase score belong to the re-verification pass, not to this edit.

**Worth one visual re-check by the host, and only one.** Fix A changes what the entry surface
looks like — it now runs edge to edge with no page background either side of it and no page
scroll of its own. `04-HUMAN-UAT.md` is `status: complete` and its verdict stands: item (a)
tested secrecy and legibility, neither of which this touches, and no UAT item is reopened by
this edit. A glance at the entry surface on a wide monitor is the only thing the automated
cases cannot stand in for, because a class name is what they can assert and a layout is not.

---

_Verified: 2026-08-25T22:36:55Z_
_Verifier: Claude (gsd-verifier)_
_Gap resolution recorded: 2026-08-25, commits `a603e55` and `032e4fa`_
